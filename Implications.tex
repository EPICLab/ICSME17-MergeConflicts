...
However, not all developers are actively using such techniques~\cite{phillips2011branching}.
We also find that practitioners experience trust issues with their merging tools (see Section~\ref{tool_trust}), which could be alleviated for developers through efforts to provide toolset training during the onboarding process when newcomers are joining a team.
These education efforts are likely to be most effective when senior members of the development team are involved in introducing newcomers to the tools and processes employed by the team.