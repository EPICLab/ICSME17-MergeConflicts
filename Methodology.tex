...
\textit{Analysis:} Interviews were unitized into cards that each contained a single logically consistent statement. This standardized coding scheme was created by the first and second author, and improved to an acceptable point by measure of intercoder \todo{mention what was the number - acceptable point is not acceptable} agreement~\cite{unitization}. The coding scheme dictated that \todo{what are these lines} lines must be consecutive and topically related to be grouped into a single card. Logically connected statements that were separated by other lines were considered to be separate cards, as a conservative measure to preserve context within each card.
