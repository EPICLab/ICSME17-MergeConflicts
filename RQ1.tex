...
\todo{say 226 people}
\todo{See what Nick thinks about this change to the following paragraphs}
...
\todo{remove std. dev from tables, bold things that we will talk about}
...
\end{tabularx}
\end{table*}
\todo{Anita says fluff-to-fact transition is what makes this rough. Talk about why these factors were shown.}

\todo{
Typical proxies used by "lay" developers are used to represent difficult code (cyclomatic complexity, atomicity, number of lines of code, number of files, etc.), but are not seen as high factors (table) according to developers. what was important was the developer's perception of the complexity of the code that they had to fix in a merge conflict.

what we need are metrics that can capture the complexity of the code which is human-understandable
